...
def myPages(canvas, doc): 
    ...
    canvas.setFillColor('#802b03') 
    canvas.rect(pageWidth-0.7*inch, 0.65*inch, 0.1*inch, 0.03*inch, fill=1, stroke=0) 
    canvas.setFillColor('#cc5a03') 
    canvas.rect(pageWidth-0.7*inch, 0.75*inch, 0.1*inch, 0.03*inch, fill=1, stroke=0) 
    canvas.setFillColor('#f39929') 
    canvas.rect(pageWidth-0.7*inch, 0.85*inch, 0.1*inch, 0.03*inch, fill=1, stroke=0) 

    canvas.setFillColor('#303030') 
    canvas.setFont('Helvetica',6) 
    canvas.drawString(pageWidth-0.55*inch, 0.65*inch, "%3.2f" % (short_max*0.33)) 
    canvas.drawString(pageWidth-0.55*inch, 0.75*inch, "%3.2f" % (short_max*0.66)) 
    canvas.drawString(pageWidth-0.55*inch, 0.85*inch, "%3.2f" % (short_max*1.00)) 
    canvas.drawString(pageWidth-0.7*inch, 0.97*inch, "%s" % ('Short freq')) 

    longStr = 'Ee'
    canvas.setFillColor('#808080') 
    canvas.setFont('Helvetica',8) 
    canvas.drawString(pageWidth-0.5*inch, 1.32*inch, "%s" % (longStr)) 
    canvas.setFont('Helvetica',10) 
    canvas.drawString(pageWidth-0.5*inch, 1.45*inch, "%s" % (longStr)) 
    canvas.setFont('Helvetica',12) 
    canvas.drawString(pageWidth-0.5*inch, 1.60*inch, "%s" % (longStr)) 
    canvas.setFont('Helvetica',14) 
    canvas.drawString(pageWidth-0.5*inch, 1.775*inch, "%s" % (longStr)) 
    canvas.setFont('Helvetica',6) 

    canvas.setFillColor('#303030') 
    canvas.drawString(pageWidth-0.7*inch, 1.32*inch, "%3.2f" % (0.25)) 
    canvas.drawString(pageWidth-0.7*inch, 1.45*inch, "%3.2f" % (0.50)) 
    canvas.drawString(pageWidth-0.7*inch, 1.60*inch, "%3.2f" % (0.75)) 
    canvas.drawString(pageWidth-0.7*inch, 1.775*inch, "%3.2f" % (1.00)) 
    canvas.drawString(pageWidth-0.7*inch, 1.19*inch, "%s" % ('Long freq')) 

    canvas.restoreState()
    
    
pageinfo = "Size/Color v5 (Preliminary Data)" 
dateNow = datetime.datetime.now().strftime("%d %b %Y")
styles = getSampleStyleSheet()
pageWidth, pageHeight = letter
doc = SimpleDocTemplate('DocFreq_PrelimData_rev5c.pdf', pagesize = letter)
...
for id in labels_list:
    ...
    dict['lo_' + id] = '%3.2f' % (long_data[id])
    # dict['sh_' + id] = '%3.2f' % (fontSize)
    if (short_data[id] >= 0):
        dict['sh_' + id] = '%3.2f' % (short_data[id])
    else:
        dict['sh_' + id] = '-'
