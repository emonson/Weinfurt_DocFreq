...
def myPages(canvas, doc): 
    ...
    canvas.drawCentredString(pageWidth/2.0, 0.65*inch, "%s" % dateNow)
    canvas.setFont('Helvetica',10) 
    canvas.drawString(pageWidth-1.0*inch, 0.65*inch, "%d" % (doc.page-1))	# Skip 1st pg number 
...
outfileName = 'DocFreq_July2009.pdf'
...
if WRITE_DATA_VALUES:
    outfileName = outfileName.replace('2009','2009_Data')

pageinfo = "Size/Color v7 (28 May 2009 Data)" 
...
long_book = xlrd.open_workbook('Data for visualization_long forms.xls')
...
short_book = xlrd.open_workbook('Data for visualization_short forms.xls')
...
for id in labels_list:
    ...
    if WRITE_DATA_VALUES:
		# if doing data labels
		dict['lo_' + id] = '%d' % (int(round(100*long_data[id])))
		# dict['sh_' + id] = '%3.2f' % (fontSize)
		if (short_data[id] >= 0):
			dict['sh_' + id] = '%d' % (int(round(100*short_data[id])))
		else:
			dict['sh_' + id] = '-'
...
f = open('LongVer_newLabelsPrelim3_rev7.html','r')
# If doing data labels
...
if WRITE_DATA_VALUES:
	...
	for ii,xx in enumerate(whole_fsp[1:-1]):
		...
		if dlStart < 0:		# line without data
			continue
...
for line in wlines:
    ...
    if line == "":
        continue
    elif 'P_A_G_E_B_R_E_A_K' in line:
        b = PageBreak()
        Story.append(b)
        del textList[:] 
    elif line.endswith('</para>'):
        p = Paragraph(''.join(textList), style)
        # print ''.join(textList)
        Story.append(p) 
        del textList[:] 
    
doc.build(Story, onFirstPage=myFirstPage, onLaterPages=myPages)
